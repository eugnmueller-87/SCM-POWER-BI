...
> prod API — so each board reflects only its own environment's data.
>
> **Credentials & writes.** The demo runs on a **public sandbox account** (`admin@example.com`
> / `admin`) against the demo API — safe to expose because it's throwaway synthetic data. The
> cockpit is effectively read-only: the one write path (the Autonomy tab's purchasing run) is
> **fail-closed** — the proxy forces `dry_run=true` unless `ALLOW_LIVE_PLACE=true` is explicitly
> set, so it can never place a live PO by accident. In production you'd swap the sandbox login
> for a dedicated read-scoped service account.
...
> Backtested across **720 forecasts over 5 years (2022–2026)**. Pooled, volume-weighted
> **accuracy ≈ 58%** (WMAPE), with a near-zero bias (**−2.6%**). The pooled figure is dragged
> down by two intermittent, near-zero-demand categories (GPU, Compute) where any miss is huge
> in % terms — the **steady, high-volume categories run 72–76% accurate**. This is the honest
> number, not a cherry-pick: the [Forecast tab](https://scm-power-bi-production.up.railway.app)
> shows it per category and per year, and explains the weak spots.

| Category | Accuracy (1 − WMAPE) | Read |
|---|---|---|
| Processors | 76% | 🟢 good |
| Memory | 76% | 🟢 good |
| Servers | 75% | 🟢 good |
...
| Storage | 51% | 🟡 watch |
| Networking | 47% | 🔴 weak |
| GPU | ~0% | 🔴 intermittent — sporadic near-zero demand |
| Compute | ~0% | 🔴 intermittent — sporadic near-zero demand |

> *Numbers are live and will shift as the dataset updates; they were current at the last README refresh. The dashboard is always authoritative.*
...
>
> **Two synthetic worlds, one project.** The **CSV world** ([`data/raw/`](data/raw/), categories
> like *IT & Cloud, Logistics, Marketing*) is generated by [`scripts/generate_data.py`](scripts/generate_data.py)
> and feeds the **Power BI lab deliverable**. The **live-API world** (categories like *Servers,
> GPU, Networking*) is served by the deployed SCM Master backend and feeds the **web cockpit**.
> They're two independent synthetic datasets — same *purpose*, different generators — so don't be
> surprised the category names differ between the `.pbix` and the live board.
...
3. **A live web cockpit** ([hosted here](https://scm-power-bi-production.up.railway.app)) —
   ...
   - **Environment-aware deployment** — the same cockpit deploys to a **demo** stack and an
     ...
     `API_BASE` (demo uses a public sandbox login; writes are fail-closed).
...
5. **A decision layer.** Every page carries a "robust vs. needs-validation" note that rolls
   ...
   answer, including where the model is weak (GPU/Compute — sporadic, near-zero demand — are the worst categories).
...
- > **🟡 Recommendation for Cleo: RUN A PILOT (~9 weeks to a go/no-go decision, ~11 to rollout) — don't invest at full scale yet, don't wait.**
...
│   └── timeline_estimate.md    # ~9wk to decision, ~11 to rollout
...
- [x] **Per-environment deployment** — same cockpit deploys to a **demo** stack and an isolated **production** stack (own Railway project), each wired to its environment's SCM Master API via `API_BASE` (demo uses a public sandbox login; writes are fail-closed)
